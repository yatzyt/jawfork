  ...
  tryCatch(
    {
      ...
      u__button(
        ...
        call_back_fct = function(widget, event, timeline, data) {
          session_name <- data[[1]]
          outer_env <- data[[2]]
          ...
          return(FALSE)
        }, data = list(session_name, outer_env, timeline, data)
      )
      ...
      refresh <- function(session_name, outer_env = totem) {
        ...
        title <- paste0(
          ...
          " | ", outer_env[[session_name]]$sas_file_path, " | ", "Ver 1.0.4.2", " | ", as.character(Sys.time())
        )
